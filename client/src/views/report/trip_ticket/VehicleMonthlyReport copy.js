...
const VehicleMonthlyReport = ({ cardTitle }) => {
  ...
  const filter = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      setProductType(selectedLabel)

      const isoDateString = values.purchase_date
      ...
      await filterSummaryConsumption.mutate({ ...values, purchase_date: formattedDate })
    },
  })
  ...
  const fFuelPumpDispenseSummaryProduct = useQuery({
    queryFn: async () =>
    ...
    queryKey: ['fFuelPumpDispenseSummaryProduct'],
    staleTime: Infinity,
  })
    ...
    <CRow>
      <CCol md={4}>
        ...
        <CForm onSubmit={filter.handleSubmit}>
          ...
          <CFormLabel>{requiredField('Date')}</CFormLabel>
          <CDatePicker
            footer
            date=""
            locale="en-US"
          ...
          <CFormLabel>
            {
              <>
                {fFuelPumpDispenseSummaryProduct.isLoading && <CSpinner size="sm" />}
          ...
          <CFormSelect
            ...
            size="sm"
            value={filter.values.product_type}
            name="product_type"
            onChange={(e) => {
          ...
          >
            ...
            {!fFuelPumpDispenseSummaryProduct.isLoading &&
              fFuelPumpDispenseSummaryProduct?.data.map((item, index) => (
                    ...
                    <View style={styles.tableRow}>
                      <View style={styles.tableColSpan}>
                        <Text style={styles.tableCellHeader}>
                          SUMMARY OF DAILY FUEL PUMP DISPENSED ({productType})
                        </Text>
                        <Text style={{ ...styles.tableCellHeader, fontSize: 10 }}>
                          {filterTitle}
                        </Text>
                      </View>
                    </View>
...
export default VehicleMonthlyReport
