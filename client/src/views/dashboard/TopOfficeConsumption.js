...
const TopOfficeConsumption = () => {
  ...
  const filterSummaryConsumption = useMutation({
    mutationKey: ['filterTopOfficeConsumption'],
    mutationFn: async (values) => {
      const response = await api.get('office/top_consumption', {
        params: values,
      })
      return response
    },
    onSuccess: async (response) => {
      await queryClient.setQueryData(['topOfficeConsumption'], response.data)
    },
    onError: (error) => {
  ...
  const removeFilter = () => {
    queryClient.invalidateQueries({ queryKey: ['topOfficeConsumption'] })
