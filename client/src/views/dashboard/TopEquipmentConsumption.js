...
const TopEquipmentConsumption = () => {
  ...
  const filterSummaryConsumption = useMutation({
    mutationKey: ['filterTopEquipmentConsumption'],
    mutationFn: async (values) => {
      const response = await api.get('equipment/top_consumption', {
        params: values,
      })
      return response
    },
    onSuccess: async (response) => {
      await queryClient.setQueryData(['topEquipmentConsumption'], response.data)
    },
    onError: (error) => {
  ...
  const removeFilter = () => {
    queryClient.invalidateQueries({ queryKey: ['topEquipmentConsumption'] })
