...
const OldTripTicket = ({ cardTitle }) => {
  ...
  const oldTripTicketProduct = useQuery({
    queryFn: async () =>
    ...
    queryKey: ['oldTripTicketProduct'],
    staleTime: Infinity,
    refetchInterval: 1000,
  })
  const oldTripTicketDriver = useQuery({
    queryFn: async () =>
    ...
    queryKey: ['oldTripTicketDriver'],
    staleTime: Infinity,
    refetchInterval: 1000,
  })
  const oldTripTicketEquipment = useQuery({
    queryFn: async () =>
    ...
    queryKey: ['oldTripTicketEquipment'],
    staleTime: Infinity,
    refetchInterval: 1000,
  })

  const oldTripTicket = useQuery({
    queryFn: async () =>
      await api.get('old_trip_ticket').then((response) => {
        return response.data
      }),
    queryKey: ['oldTripTicket'],
    staleTime: Infinity,
  })
  ...
  const form = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      if (values.id) {
        await updateOldTripTicket.mutate(newValues)
      } else {
        await insertOldTripTicket.mutate(newValues)
      }
    },
  })

  const insertOldTripTicket = useMutation({
    mutationFn: async (values) => {
      return await api.post('old_trip_ticket/insert', values)
    },
    onSuccess: async (response) => {
      ...
      await queryClient.invalidateQueries(['OldTripTicket'])
    },
    onError: (error) => {
      console.info(error)
  ...
  const updateOldTripTicket = useMutation({
    mutationFn: async (values) => {
      return await api.put('old_trip_ticket/update/' + values.id, values)
    },
    onSuccess: async (response) => {
      if (response.data.status) {
        toast.success(response.data.message)
  ...
  return (
    <>
      ...
      <MaterialReactTable
        ...
        state={{
          isLoading:
            oldTripTicket.isLoading ||
            insertOldTripTicket.isPending ||
            updateOldTripTicket.isPending,
          isSaving:
            oldTripTicket.isLoading ||
            insertOldTripTicket.isPending ||
            updateOldTripTicket.isPending,
                  ...
                  Swal.fire({
                    ...
                    if (result.isConfirmed) {
                      validationPrompt(async () => {
                        ...
                        await api
                          .delete('old_trip_ticket/delete/' + id)
                          .then(async (response) => {
                            await queryClient.invalidateQueries(['OldTripTicket'])
      ...
      >
        ...
        <CModalBody>
                ...
                <CRow>
                  ...
                  <CCol md={12}>
                    ...
                    <Select
                      ...
                      value={
                        ...
                        oldTripTicketProduct.data?.find(
                          (option) => option.value === form.values.product,
                        )
                      }
                      onChange={handleSelectChange}
                      options={!oldTripTicketProduct.isLoading && oldTripTicketProduct.data}
                      name="product"
                      isSearchable
                      placeholder="Search..."
                      isClearable
                  ...
                  <CCol md={12}>
                    <CFormLabel>
                      {
                        <>
                          {oldTripTicketDriver.isLoading && <CSpinner size="sm" />}
                    ...
                    <Select
                      ...
                      value={
                        !oldTripTicketDriver.isLoading &&
                        oldTripTicketDriver.data?.find(
                          (option) => option.value === form.values.driver,
                        )
                      }
                      onChange={handleSelectChange}
                      options={!oldTripTicketDriver.isLoading && oldTripTicketDriver.data}
                      name="driver"
                      isSearchable
                      placeholder="Search..."
                      isClearable
                  ...
                  <CCol md={12}>
                    <CFormLabel>
                      {
                        <>
                          {oldTripTicketEquipment.isLoading && <CSpinner size="sm" />}
                    ...
                    <Select
                      ...
                      value={
                        !oldTripTicketEquipment.isLoading &&
                        oldTripTicketEquipment.data?.find(
                          (option) => option.value === form.values.equipment,
                        )
                      }
                      onChange={handleSelectChange}
                      options={!oldTripTicketEquipment.isLoading && oldTripTicketEquipment.data}
                      name="equipment"
                      isSearchable
                      placeholder="Search..."
                      isClearable
          ...
          {(insertOldTripTicket.isPending || updateOldTripTicket.isPending) && <DefaultLoading />}
        </CModalBody>
      </CModal>
    </>
  )
